...
function entrySignature(allDefs, registry, storageEntry, imports) {
  ...
  if (storageEntry.type.isPlain) {
    ...
    const key1 = (0, _util2.getSimilarTypes)(allDefs, registry, storageEntry.type.asDoubleMap.key1.toString(), imports);
    const key2 = (0, _util2.getSimilarTypes)(allDefs, registry, storageEntry.type.asDoubleMap.key2.toString(), imports);
    const value = storageEntry.type.asDoubleMap.value.toString();
    (0, _util2.setImports)(allDefs, imports, [...key1, ...key2, value]);
    return [[key1.map(format).join(' | '), key2.map(format).join(' | ')], (0, _util2.formatType)(allDefs, outputType, imports)];
  }
