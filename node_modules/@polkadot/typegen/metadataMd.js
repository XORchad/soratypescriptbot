...
function addRuntime(_runtimeDesc, apis) {
  return renderPage({
    ...
    sections: Object.keys(definitions).filter(key => Object.keys(definitions[key].runtime || {}).length !== 0).sort().reduce((all, _sectionName) => {
      Object.entries(definitions[_sectionName].runtime || {}).forEach(([apiName, versions]) => {
        versions.sort((a, b) => b.version - a.version).forEach(({
          methods,
          version
        }, index) => {
          if (apis) {
          ...
          all.push(container);
          Object.entries(methods).sort(([a], [b]) => a.localeCompare(b)).forEach(([methodName, {
            description,
            params,
            type
          }]) => {
...
/** @internal */
function addConstants(runtimeDesc, {
  lookup,
  pallets
}) {
  return renderPage({
...
/** @internal */
function addStorage(runtimeDesc, {
  lookup,
  pallets,
  registry
}) {
...
/** @internal */
function addExtrinsics(runtimeDesc, {
  lookup,
  pallets
}) {
  return renderPage({
...
/** @internal */
function addEvents(runtimeDesc, {
  lookup,
  pallets
}) {
  return renderPage({
...
/** @internal */
function addErrors(runtimeDesc, {
  lookup,
  pallets
}) {
  return renderPage({
