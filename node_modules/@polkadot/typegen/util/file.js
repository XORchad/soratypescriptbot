...
export function readTemplate(template) {
  ...
  const rootDir = packageInfo.path === 'auto' ? path.join(process.cwd(), 'packages/typegen/src') : packageInfo.path;

  // NOTE With cjs in a subdir, search one lower as well
  const file = ['./templates', '../templates'].map(p => path.join(rootDir, p, `${template}.hbs`)).find(p => fs.existsSync(p));
  if (!file) {
